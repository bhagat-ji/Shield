...
/**
 * A manager singleton that holds all the [SHIELDView] and tracks the current tab. It handles
...
class TabsManager @Inject constructor(
    ...
    private val tabList = arrayListOf<SHIELDView>()

    /**
     * Return the current [SHIELDView] or null if no current tab has been set.
     *
     * @return a [SHIELDView] or null if there is no current tab.
     */
    var currentTab: SHIELDView? = null
        private set
     ...
     */
    fun initializeTabs(activity: Activity, intent: Intent?, incognito: Boolean): Single<SHIELDView> =
        Single
    ...
    private fun initializeIncognitoMode(initialUrl: String?): Observable<TabInitializer> =
        Observable.fromCallable {
            return@fromCallable initialUrl?.let(::UrlInitializer) ?: incognitoPageInitializer
        }
    ...
    fun pauseAll() {
        ...
        tabList.forEach(SHIELDView::onPause)
    }
     ...
     * @return the corespondent [SHIELDView], or null if the index is invalid
     */
    fun getTabAtPosition(position: Int): SHIELDView? =
     ...
     */
    fun lastTab(): SHIELDView? = tabList.lastOrNull()
     ...
     */
    fun positionOf(tab: SHIELDView?): Int = tabList.indexOf(tab)
     ...
     */
    fun indexOfTab(tab: SHIELDView): Int = tabList.indexOf(tab)

    /**
     * Returns the [SHIELDView] with the provided hash, or null if there is no tab with the hash.
     *
     * @param hashCode the hashcode.
     * @return the tab with an identical hash, or null.
     */
    fun getTabForHashCode(hashCode: Int): SHIELDView? =
     ...
     */
    fun switchToTab(position: Int): SHIELDView? {
