Some SHIELD features use online services. Below, the data collected and used by these is detailed.

# News

SHIELD offers a homepage news feed.

The feed is generated using NewsAPI. There is a piece of server side code on the SHIELD website that looks up your IP in a database to guess the country you are in, and then calls the API using that to provide localised news. The IP is not stored anywhere or sent to 3rd parties. Localised news can be disabled in settings.
...
SHIELD offers a custom search suggestion system (disabled by default). Your IP address is used to provide results relevant to your country. Queries made to the server for suggestions are not stored.
